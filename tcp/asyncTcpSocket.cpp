    ...
    namespace net
    {
        ...
        void AsyncTcpSocket::Write(const std::string &data)
        {
            ...
            if (m_loop->IsInLoopTread())
            {
                ...
                int32_t nwrite =
                    TcpSocket::Write(GetNativeSocket(), data.data(), static_cast<uint32_t>(data.size()));
                if (nwrite >= 0)
                {
        ...
        int32_t AsyncTcpSocket::HandleWrite()
        {
            ...
            do
            {
                ...
                nwrite = TcpSocket::Write(GetNativeSocket(),
                                          ...
                                          static_cast<uint32_t>(m_writeCache.size() - m_writedCacheLen));
                if (nwrite >= 0)
                {
        ...
        int32_t AsyncTcpSocket::HandleRead()
        {
            ...
            while (true)
            {
                ...
                {
                    m_read_cb(shared_from_this(), buffer);
                }
            }
