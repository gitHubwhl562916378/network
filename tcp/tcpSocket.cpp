...
namespace geely
{
    namespace net
    {
        ...
        int32_t TcpSocket::Write(const std::string &data)
        {
            ...
            return Write(GetNativeSocket(), data.data(), static_cast<int32_t>(data.size()));
        }
        int32_t TcpSocket::Read(std::string &data)
        {
            ...
            auto bytes = Read(GetNativeSocket(), const_cast<char *>(data.data()), static_cast<int32_t>(data.size()));
            if (0 >= bytes)
            {
                return bytes;
            }
        ...
        {
            return static_cast<int32_t>(::write(fd, buffer, bufferLen));
        }
        int32_t TcpSocket::Read(const int32_t fd, char *buffer, const int32_t bufferLen)
        {
            return static_cast<int32_t>(::read(fd, buffer, bufferLen));
        }
    } // namespace net
} // namespace geely