...
namespace geely
{
    namespace net
    {
        class LIB_EXPORT UdpServer : public AsyncUdpSocket
        {
        public:
            /**
         ...
         * @param loop IO事件循环
         */
            explicit UdpServer(std::shared_ptr<IoLoop> loop);
            /**
         * @brief Destroy the Udp Server object
         */
            ~UdpServer();
            /**
         * @brief 绑定端口,需要使用Read把OnClientMessage设置为读取回调
         * @param host 端口和绑定地址,需要使用SetHost保存到基类，然后保存到m_localHost用于记录服务绑定信息，基类的GetHost记录客户端信息
         * @return true 成功
         * @return false 失败
         */
