...
namespace geely {
class TimerSocket : public net::AsyncSocket, public std::enable_shared_from_this<TimerSocket> {
public:
    using TimeoutFunctor = std::function<void(const uint64_t)>;
    /**
     * @brief Construct a new Timer Socket object
     */
    explicit TimerSocket();
    /**
     * @brief Construct a new Timer Socket object
     * @param firstInterval 第一次超时的时间(毫秒)，0表示不启用，计时器不会触发
     * @param intervalAfterFirst 第一次超时后，后面每一次超时的间隔(毫秒)，0表示第一次超时后结束，总共只超时一次
     */
    explicit TimerSocket(const uint32_t firstInterval, const uint32_t intervalAfterFirst = 0);
    /**
